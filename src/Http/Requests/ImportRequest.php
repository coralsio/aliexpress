...
namespace Corals\Modules\Aliexpress\Http\Requests;
...
class ImportRequest extends BaseRequest
{
    ...
    public function rules()
    {
        ...
        if ($this->isStore()) {
            $rules = array_merge($rules, [
                'title' => 'required|max:191|unique:aliexpress_imports,title',
            ]);
        }
        ...
        if ($this->isUpdate()) {
            ...
            $rules = array_merge($rules, [
                'title' => 'required|max:191|unique:aliexpress_imports,title,' . $import->id,
            ]);
        }
